...
APP_FW_START                = 0x8000
APP_FW_END                  = FLASH_SIZE
APP_FW_INFO                 = APP_FW_END - 4
APP_FW_LENGTH               = APP_FW_INFO - APP_FW_START

#Renesas has heterogenous row sizes in program flash, as well as data flash
FLASH_ROW_SIZE_32K          = 32 * 1024
FLASH_ROW_SIZE_8K           = 8 * 1024
FLASH_ROW_SIZE_SWITCH       = 64 * 1024 #This is where flash rows switch layout from 8K to 32K
DATA_FLASH_START            = 0x08000000
DATA_FLASH_END              = 0x08000000 + 4 * 1024
DATA_FLASH_ROW_SIZE          = 64
#Logical size for storing data
DATA_FLASH_LOGICAL_SIZE       = 256

INFO_OFFSET                 = DATA_FLASH_START
BLOB_OFFSET                 = INFO_OFFSET + DATA_FLASH_LOGICAL_SIZE
...
class DogBootloaderMTEBlob:
    STRUCT = struct.Struct(f"<IB{DATA_FLASH_LOGICAL_SIZE - CRCLEN - 2}s" +
                           "B" # NULL termination
                           )
    assert(STRUCT.size == DATA_FLASH_LOGICAL_SIZE)
    ...
    def __init__(self, blob):
        if isinstance(blob, str):
            ...
            valid = compute_crc(blob[CRCLEN:], DATA_FLASH_LOGICAL_SIZE - 4) == crc
    ...
    def __bytes__(self):
        ...
        blob     = self.STRUCT.pack(0x00000000,
                                    ...
                                    mte_blob,
                                    0x00)
        return self.STRUCT.pack(compute_crc(blob[CRCLEN:], DATA_FLASH_LOGICAL_SIZE - 4),
                                0x00,
                                mte_blob,
                                0x00)
...
class DogBootloaderDeviceInfo:
    def __init__(self, blob):
        ...
        if magic != DEVICE_INFO_MAGIC     or \
           ...
           crc   != compute_crc(blob[CRCLEN:], DATA_FLASH_LOGICAL_SIZE - 4):
            board_serial = bytes()
            unit_serial  = bytes()
    ...
    def __bytes__(self):
        ...
        padding = bytes(0xFF for _ in range(0, DATA_FLASH_LOGICAL_SIZE - self.struct.size))
        blob    = self.struct.pack(0x00000000,
                                   DEVICE_INFO_MAGIC,
...
class DogBootloader:
    ...
    def get_row_size(self, offset):
        if offset >= DATA_FLASH_START and offset < DATA_FLASH_END:
            return DATA_FLASH_ROW_SIZE
        elif offset  < FLASH_ROW_SIZE_SWITCH:
            return FLASH_ROW_SIZE_8K
        elif offset >= FLASH_ROW_SIZE_SWITCH and offset < FLASH_END:
            return FLASH_ROW_SIZE_32K
        else:
            return 0
    ...
    def erase(self):
        base_addr = APP_FW_START
        while(base_addr < APP_FW_END):
            row_size = self.get_row_size(base_addr)
            assert(row_size != 0)

            self.erase_row(base_addr)
            base_addr += row_size

    def read_row(self, offset):
        row = bytes()
        row_size = self.get_row_size(offset)
        for _ in range(0, row_size, 32):
    ...
    def update_crc(self, crc):
        ...
        offset = APP_FW_END - self.get_row_size(APP_FW_INFO)
    ...
    def upload_firmware(self, name, populate_crc=True):
        ...
        #Pad the new application to the full size, then add CRC
        blob += bytes([0xFF] * (APP_FW_LENGTH - len(blob)))
        if populate_crc:
            blob+= struct.pack("<I", compute_crc(blob, APP_FW_LENGTH))
        else:
            blob+=bytes([0xFF]*4)
        
        #Always erase all appliction flash for consistency, then write the new data
        self.erase()
        
        LOG.info(f"Uploading {name} to {self}, size: {len(blob)}")
        for offset in range(0,len(blob), 32):
            self.write_32b(APP_FW_START+offset, blob[offset : offset + 32])
    ...
    def read_data_rows(self, offset):
        """ Read the couple of data rows that gives us the logical storage unit """
        row = bytes()
        for _ in range(0, DATA_FLASH_LOGICAL_SIZE, 32):
            row    += self.read_32b(offset)
            offset += 32
        #
        # Needs to be bytearray so it would be modifiable
    ...
    def update_data_rows(self, offset, data):
        assert(len(data) == DATA_FLASH_LOGICAL_SIZE)
        base_addr = offset
        while(base_addr < offset + DATA_FLASH_LOGICAL_SIZE):
            row_size = self.get_row_size(base_addr)
            self.erase_row(base_addr)
            self.write_row(base_addr, data[0:row_size])
            base_addr += row_size
            data = data[row_size:]

    @property
    def info(self):
        return DogBootloaderDeviceInfo(self.read_data_rows(INFO_OFFSET))

    @info.setter
    def info(self, value):
        assert type(value) is DogBootloaderDeviceInfo
        self.update_data_rows(INFO_OFFSET, bytes(value))
    ...
    @property
    def mte_blob(self):
        return DogBootloaderMTEBlob(self.read_data_rows(BLOB_OFFSET))

    @mte_blob.setter
    def mte_blob(self, val):
        self.update_data_rows(BLOB_OFFSET,
